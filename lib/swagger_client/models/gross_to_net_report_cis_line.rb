  ...
  class GrossToNetReportCisLine
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::GrossToNetReportCisLine` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::GrossToNetReportCisLine`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'labour')
        self.labour = attributes[:'labour']
      end

      if attributes.key?(:'materials')
        self.materials = attributes[:'materials']
      end
    ...
    def hash
      [employee, payroll_code, department, total_gross, labour, materials, taxable_gross, cis_deduction, umbrella_fee, vat, payment].hash
    end
