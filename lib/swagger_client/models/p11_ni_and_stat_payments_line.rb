  ...
  class P11NiAndStatPaymentsLine
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::P11NiAndStatPaymentsLine` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::P11NiAndStatPaymentsLine`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'class1a')
        self.class1a = attributes[:'class1a']
      end

      if attributes.key?(:'ni_values')
        self.ni_values = attributes[:'ni_values']
      end
    ...
    def hash
      [date, period, month_number, week_number, smp, spp, sap, shpp, ssp, spbp, class1a, ni_values, tax_code].hash
    end
