  ...
  class CisSubContractorSummary
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::CisSubContractorSummary` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::CisSubContractorSummary`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'validation_message')
        if (value = attributes[:'validation_message']).is_a?(Array)
    ...
    def hash
      [payroll_code, trading_name, first_name, last_name, ni_number, type, utr, tax_status, verification_number, display_name, validation_message].hash
    end
