  ...
  class AnalysisReport
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::AnalysisReport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::AnalysisReport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pay_codes')
        if (value = attributes[:'pay_codes']).is_a?(Array)
          self.pay_codes = value
        end
      end
    ...
    def hash
      [lines, pay_codes, employer, pay_period, ordinal, period, period_to, start_period_name, end_period_name, start_date, end_date, report, tax_year].hash
    end
