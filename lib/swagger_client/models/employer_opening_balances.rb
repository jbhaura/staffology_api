  ...
  class EmployerOpeningBalances
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::EmployerOpeningBalances` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::EmployerOpeningBalances`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [smp_recovered, spp_recovered, sap_recovered, sh_pp_recovered, spbp_recovered, nic_compensation_on_smp, nic_compensation_on_spp, nic_compensation_on_sap, nic_compensation_on_sh_pp, nic_compensation_on_spbp, cis_deductions_suffered].hash
    end
